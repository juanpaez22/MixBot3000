...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_BLUE,ST7735_WHITE, 1);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_WHITE, 1);
...
/**************ST7735_XYplotInit***************
 Specify the X and Y axes for an x-y scatter plot by setting 
...
*/
int32_t global_minX, global_maxX, global_minY, global_maxY;

void ST7735_XYplotInit(char *title, int32_t minX, int32_t maxX, int32_t minY, int32_t maxY){
	ST7735_OutString("\r");
	ST7735_OutString(title);
	ST7735_PlotClear(32, 160);
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[]){
  uint16_t i;
	int32_t plotX, plotY;
	int32_t rangeX = global_maxX - global_minX; 
	int32_t rangeY = global_maxY - global_minY;
	
	for(i = 0; i < num; i++) {
		if (bufX[i] + 1 <= global_maxX && bufX[i] >= global_minX && bufY[i] + 1 <= global_maxY && bufY[i] >= global_minY) {
			plotX = ((bufX[i] - global_minX) << 7) / rangeX;
			plotY = 32 + (((global_maxY - bufY[i]) << 7) / rangeY);
			ST7735_DrawPixel(plotX + 1, plotY + 1, ST7735_BLUE);
			ST7735_DrawPixel(plotX + 1, plotY, ST7735_BLUE);
			ST7735_DrawPixel(plotX, plotY + 1, ST7735_BLUE);
...
// *************** ST7735_DisplayRatioBar ********************
// Used to display vertical ratio bar
// Creates a ratio bar that is the 160 pixels wide and given thickness and colors
// Input: y     vertical position of the top left corner of the rectangle, rows from the top edge
//        w     width of the rectangle
//				h			height of the rectangle
//        color 16-bit color, which can be produced by ST7735_Color565()
//        num   the ratio of the left side of the bar with respect to the denominator
//        denom the denominator of the ratio, max 128 resolution
// Output: none
void ST7735_DisplayRatioBar(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t colorLeft, uint16_t colorRight, uint32_t num, uint32_t denom) {
   ST7735_FillRect(x, y, w, (num * h) / denom, colorLeft);
   ST7735_FillRect(x, (num * h)/denom, w, h - (num *h) / denom, colorRight);
}
